...
def proba_extraction(k, nb_rouge, nb_blanche, nb_noire):
    ...
    combinaisons_une_de_chaque = (
        math.comb(nb_rouge, 1) * math.comb(nb_blanche, 1) * math.comb(nb_noire, 1)
    )
...
print(
    f"La probabilité d'obtenir exactement une boule de chaque couleur est : {resultat}"
    )